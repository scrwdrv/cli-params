...
class CLIParams {
    ...
    exec(cb) {
        const ids = Object.keys(this.formats), parse = (i = 0) => {
            ...
            next();
            function parseType(type, param, val, isArray = false) {
                if (isArray) {
                    type = type.slice(9);
                    if (!result[param])
                        result[param] = [];
                        ...
                        for (let j = i + 1; j < args.length; j++) {
                            if (/^-/.test(args[j])) {
                                ...
                                if (!result[param])
                                    ...
                                    result[param].push(args[j]);
                                else
                                    result[param] = [result[param], args[j]];
                                i++;
                            }
                        }
                }
